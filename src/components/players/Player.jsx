...
const Player = (props) => {
  ...
  return (
    <div className={`player-entity--wrapper p${arrayIndex}`}>
      ...
      />
      <div className={`centered-flex-row abscard number${arrayIndex}`}>
        {renderPlayerCards()}
      </div>
      <div className='player-entity--container'>
        <div className='player-avatar--container'>
          ...
          <h5
            ...
            style={{ fontSize: name.length < 14 ? 13 : 12 }}
          >
            {`${name}`}
          </h5>
          <div className={`player-info--stash--container name${arrayIndex}`}>
            <img
              className='player-info--stash--image'
              src={chipCountImageURL}
              alt='Player Stash'
            />
            <h5 className={`Amount`}>${`${chips}`}</h5>
          </div>
          <div className='player-info--bet--container'>
            <img
              ...
              alt='Player Bet'
            />
            <h5 className='TheAmountBed'>{`Bet: ${bet}`}</h5>
          </div>
          {renderDealerChip()}
        </div>
      </div>
